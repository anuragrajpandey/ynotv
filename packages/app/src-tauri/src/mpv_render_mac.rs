...
use crate::mpv_core::{map_css_geometry, MpvGeometry};
...
pub fn install(mpv_ctx: NonNull<mpv_handle>, ns_window_ptr: i64, edr: bool) -> Result<(), String> {
    ...
    if let Some(stale) = stale {
        log::info!("[ynotv::mpv_mac] replacing stale embed on the main thread");
    ...
    unsafe {
        ...
        log::info!(
            "[ynotv::mpv_mac] contentView bounds: {}x{}",
        ...
        *slot().lock().map_err(|e| format!("slot lock: {}", e))? = Some(Embed {
        ...
        log::info!("[ynotv::mpv_mac] installed native OpenGL embed successfully");
    }
    Ok(())
}
...
pub fn set_hdr_active(active: bool, _bt2020: bool) {
    if MainThreadMarker::new().is_none() {
        log::warn!("[ynotv::mpv_mac] ignored HDR update off the main thread");
        return;
    }
...
pub fn render_now() -> Result<(), String> {
    ...
    unsafe {
        ...
        if LAST_SURFACE.swap(packed, Ordering::Relaxed) != packed {
            log::info!(
                "[ynotv::mpv_mac] render surface {}x{} px (bounds {}x{} pt)",
...
pub fn uninstall() -> Result<(), String> {
    ...
    log::info!("[ynotv::mpv_mac] uninstalled");
    Ok(())
}
